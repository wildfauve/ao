...
def mens_singles_round_2(mens_singles):
    ...
    TEAM.draw(mens_singles).match('2.2').winner(men.Shapovalov).in_sets(5)  # (   ) Arnaldi  OR  ( 26) Shapovalov
    TEAM.draw(mens_singles).match('2.3').winner(men.Musetti).in_sets(4)  # ( 17) Musetti  OR  (   ) Shevchenko
    TEAM.draw(mens_singles).match('2.4').winner(men.Pouille).in_sets(4)  # (  Q) Pouille  OR  ( 14) Norrie
    TEAM.draw(mens_singles).match('2.5').winner(men.Kubler).in_sets(4)  # (   ) Fognini  OR  (   ) Kubler
    TEAM.draw(mens_singles).match('2.6').winner(men.Korda).in_sets(5)  # (  Q) Ofner  OR  ( 24) Korda
    TEAM.draw(mens_singles).match('2.7').winner(men.Schwartzman).in_sets(4)  # (   ) Schwartzman  OR  (   ) Borges
    TEAM.draw(mens_singles).match('2.8').winner(men.Carballes_Baena).in_sets(4)  # (   ) Carballes_Baena  OR  (  5) Tsitsipas
    TEAM.draw(mens_singles).match('2.9').winner(men.Djokovic).in_sets(3)  # (  3) Djokovic  OR  (   ) Fucsovics
    TEAM.draw(mens_singles).match('2.10').winner(men.Davidovich_Fokina).in_sets(4)  # (   ) Van_Assche  OR  ( 29) Davidovich_Fokina
    TEAM.draw(mens_singles).match('2.11').winner(men.Bautista_Agut).in_sets(5)  # ( 19) Bautista_Agut  OR  (   ) Varillas
    ...
    TEAM.draw(mens_singles).match('2.14').winner(men.Wawrinka).in_sets(5)  # (   ) Wawrinka  OR  (  W) Kokkinakis
    TEAM.draw(mens_singles).match('2.15').winner(men.Humbert).in_sets(5)  # (   ) Sonego  OR  (   ) Humbert
    TEAM.draw(mens_singles).match('2.16').winner(men.Moutet).in_sets(5)  # (   ) Moutet  OR  (  7) Rublev
    TEAM.draw(mens_singles).match('2.17').winner(men).in_sets()  # (  6) Rune  OR  (   ) Monfils
    TEAM.draw(mens_singles).match('2.18').winner(men).in_sets()  # (  Q) Olivieri  OR  (  Q) Vavassori
    TEAM.draw(mens_singles).match('2.19').winner(men).in_sets()  # ( 23) Cerundolo_Francisco  OR  (  L) Hanfmann
    TEAM.draw(mens_singles).match('2.20').winner(men).in_sets()  # (   ) Rinderknech  OR  (  9) Fritz
    TEAM.draw(mens_singles).match('2.21').winner(men).in_sets()  # ( 16) Paul  OR  (   ) Jarry
    ...
    TEAM.draw(mens_singles).match('2.24').winner(men).in_sets()  # (  Q) Zeppieri  OR  (  4) Ruud
    TEAM.draw(mens_singles).match('2.25').winner(men).in_sets()  # (  8) Sinner  OR  (   ) Altmaier
    TEAM.draw(mens_singles).match('2.26').winner(men).in_sets()  # (   ) Ruusuvuori  OR  ( 28) Dimitrov
    TEAM.draw(mens_singles).match('2.27').winner(men).in_sets()  # ( 22) Zverev  OR  (   ) Molcan
    ...
    TEAM.draw(mens_singles).match('2.31').winner(men).in_sets()  # ( 27) Nishioka  OR  (   ) Purcell
    TEAM.draw(mens_singles).match('2.32').winner(men).in_sets()  # (   ) Pella  OR  (  Q) Seyboth_Wild
    return TEAM
...
def womens_singles_round_2(womens_singles):
    ...
    TEAM.draw(womens_singles).match('2.27').winner(women.Stearns).in_sets(2)  # ( 17) Ostapenko  OR  (   ) Stearns
    TEAM.draw(womens_singles).match('2.28').winner(women.Kasatkina).in_sets(3)  # (   ) Vondrousova  OR  (  9) Kasatkina
    TEAM.draw(womens_singles).match('2.29').winner(women.Stephens).in_sets(3)  # (   ) Stephens  OR  (   ) Gracheva
    TEAM.draw(womens_singles).match('2.30').winner(women.Zheng).in_sets(2)  # (   ) Putintseva  OR  ( 19) Zheng
    TEAM.draw(womens_singles).match('2.31').winner(women.Rakhimova).in_sets(3)  # (   ) Frech  OR  (   ) Rakhimova
    TEAM.draw(womens_singles).match('2.32').winner(women.Sabalenka).in_sets(3)  # (  Q) Shymanovich  OR  (  2) Sabalenka
...
def mens_singles_round_1(mens_singles):
    ...
    TEAM.draw(mens_singles).match('1.3').winner(men.Arnaldi).in_sets(5)  # (   ) Matteo Arnaldi  OR  (   ) Daniel Elahi Galan
    ...
    TEAM.draw(mens_singles).match('1.9').winner(men.Fognini).in_sets(4)  # ( 10) Felix Auger-Aliassime  OR  (   ) Fabio Fognini
    TEAM.draw(mens_singles).match('1.10').winner(men.Kubler).in_sets(5)  # (   ) Jason Kubler  OR  (  L) Diaz Acosta
    TEAM.draw(mens_singles).match('1.11').winner(men.Cressy).in_sets(3)  # (  Q) Sebastian Ofner  OR  (   ) Maxime Cressy
    TEAM.draw(mens_singles).match('1.12').winner(men.Korda).in_sets(4)  # (   ) Mackenzie McDonald  OR  ( 24) Sebastian Korda
    ...
    TEAM.draw(mens_singles).match('1.18').winner(men.Fucsovics).in_sets(5)  # (   ) Marton Fucsovics  OR  (  W) Hugo Grenier
    TEAM.draw(mens_singles).match('1.19').winner(men.Cecchinato).in_sets(4)  # (   ) Luca Van Assche  OR  (   ) Marco Cecchinato
    TEAM.draw(mens_singles).match('1.20').winner(men.Davidovich_Fokina).in_sets(3)  # (  W) Arthur Fils  OR  ( 29) Alejandro Davidovich Fokina
    TEAM.draw(mens_singles).match('1.21').winner(men.Bautista_Agut).in_sets(3)  # ( 19) Roberto Bautista Agut  OR  (   ) Yibing Wu
    TEAM.draw(mens_singles).match('1.22').winner(men.Varillas).in_sets(5)  # (  Q) Juncheng Shang  OR  (   ) Juan Pablo Varillas
    TEAM.draw(mens_singles).match('1.23').winner(men.Griekspoor).in_sets(3)  # (  Q) Pedro Martinez  OR  (   ) Tallon Griekspoor
    TEAM.draw(mens_singles).match('1.24').winner(men.Hurkacz).in_sets(4)  # (   ) David Goffin  OR  ( 13) Hubert Hurkacz
    TEAM.draw(mens_singles).match('1.25').winner(men.Khachanov).in_sets(4)  # ( 11) Karen Khachanov  OR  (   ) Constant Lestienne
    TEAM.draw(mens_singles).match('1.26').winner(men.Kypson).in_sets(4)  # (  W) Patrick Kypson  OR  (  Q) Radu Albot
    TEAM.draw(mens_singles).match('1.27').winner(men.Ramos_Vinolas).in_sets(5)  # (   ) Stan Wawrinka  OR  (   ) Albert Ramos-Vinolas
    ...
    TEAM.draw(mens_singles).match('1.30').winner(men.Mannarino).in_sets(5)  # (   ) Adrian Mannarino  OR  (   ) Ugo Humbert
    TEAM.draw(mens_singles).match('1.31').winner(men.Moutet).in_sets(5)  # (  W) Arthur Cazaux  OR  (   ) Corentin Moutet
    ...
    TEAM.draw(mens_singles).match('1.36').winner(men.Vavassori).in_sets(4)  # (  Q) Andrea Vavassori  OR  ( 31) Miomir Kecmanovic
    ...
    TEAM.draw(mens_singles).match('1.39').winner(men.Gasquet).in_sets(5)  # (   ) Richard Gasquet  OR  (   ) Arthur Rinderknech
    ...
    TEAM.draw(mens_singles).match('1.44').winner(men.Struff).in_sets(3)  # (   ) Jiri Lehecka  OR  ( 21) Jan-Lennard Struff
    ...
    TEAM.draw(mens_singles).match('1.47').winner(men.Zeppieri).in_sets(5)  # (   ) Alexander Bublik  OR  (  Q) Giulio Zeppieri
    ...
    TEAM.draw(mens_singles).match('1.50').winner(men.Huesler).in_sets(3)  # (   ) Daniel Altmaier  OR  (   ) Marc-Andrea Huesler
    TEAM.draw(mens_singles).match('1.51').winner(men.Barrere).in_sets(3)  # (   ) Emil Ruusuvuori  OR  (   ) Gregoire Barrere
    TEAM.draw(mens_singles).match('1.52').winner(men.Dimitrov).in_sets(3)  # (  Q) Timofey Skatov  OR  ( 28) Grigor Dimitrov
    TEAM.draw(mens_singles).match('1.53').winner(men.Zverev).in_sets(5)  # ( 22) Alexander Zverev  OR  (   ) Lloyd Harris
    TEAM.draw(mens_singles).match('1.54').winner(men.Gaston).in_sets(5)  # (  W) Hugo Gaston  OR  (   ) Alex Molcan
    TEAM.draw(mens_singles).match('1.55').winner(men.Popyrin).in_sets(4)  # (   ) Alexei Popyrin  OR  (  Q) Aslan Karatsev
    TEAM.draw(mens_singles).match('1.56').winner(men.Tiafoe).in_sets(4)  # (   ) Filip Krajinovic  OR  ( 12) Frances Tiafoe
    TEAM.draw(mens_singles).match('1.57').winner(men.Coric).in_sets(3)  # ( 15) Borna Coric  OR  (   ) Federico Coria    TEAM.draw(mens_singles).match('1.58').winner(men.Thiem).in_sets(4)  # (   ) Dominic Thiem  OR  (   ) Pedro Cachin
    TEAM.draw(mens_singles).match('1.59').winner(men.Draper).in_sets(3)  # (   ) Jack Draper  OR  (   ) Tomas Martin Etcheverry
    ...
    TEAM.draw(mens_singles).match('1.64').winner(men.Medvedev).in_sets(4)  # (  Q) T.Seyboth Wild  OR  (  2) Daniil Medvedev
...
def womens_singles_round_1(womens_singles):
    ...
    TEAM.draw(womens_singles).match('1.6').winner(women.Andreeva_Erika).in_sets(3)  # (   ) Andreeva_Erika  OR  (  W) Emma Navarro
    ...
    TEAM.draw(womens_singles).match('1.13').winner(women.Parry).in_sets(2)  # ( 25) Anhelina Kalinina  OR  (  W) Diane Parry
    TEAM.draw(womens_singles).match('1.14').winner(women.Riske_Amritraj).in_sets(2)  # (  Q) Mirra Andreeva  OR  (   ) Alison Riske-Amritraj
    ...
    TEAM.draw(womens_singles).match('1.18').winner(women.Noskova).in_sets(2)  # (   ) Linda Noskova  OR  (   ) Danka Kovinic
    TEAM.draw(womens_singles).match('1.19').winner(women.Burel).in_sets(2)  # (  W) Clara Burel  OR  (   ) Sorribes Tormo
    TEAM.draw(womens_singles).match('1.20').winner(women.Rogers).in_sets(3)  # (   ) Petra Martic  OR  ( 32) Shelby Rogers
    TEAM.draw(womens_singles).match(
        '1.21').winner(women.Alexandrova).in_sets(3)  # ( 23) Ekaterina Alexandrova  OR  (   ) Viktoriya Tomova
    TEAM.draw(womens_singles).match('1.22').winner(women.Friedsam).in_sets(3)  # (  L) Nao Hibino  OR  (   ) Anna-Lena Friedsam
    TEAM.draw(womens_singles).match('1.23').winner(women.Marino).in_sets(3)  # (   ) Rebecca Marino  OR  (   ) Diana Shnaider
    TEAM.draw(womens_singles).match('1.24').winner(women.Haddad_Maia).in_sets(3)  # (   ) Tatjana Maria  OR  ( 14) Beatriz Haddad Maia
    TEAM.draw(womens_singles).match('1.25').winner(women.Cocciaretto).in_sets(2)  # ( 10) Petra Kvitova  OR  (   ) Elisabetta Cocciaretto
    TEAM.draw(womens_singles).match('1.26').winner(women.Waltert).in_sets(3)  # (  Q) Simona Waltert  OR  (  Q) Elizabeth Mandlik
    TEAM.draw(womens_singles).match('1.27').winner(women.Kontaveit).in_sets(3)  # (   ) Anett Kontaveit  OR  (   ) Bernarda Pera
    TEAM.draw(womens_singles).match('1.28').winner(women.Vekic).in_sets(3)  # (  Q) Dayana Yastremska  OR  ( 22) Donna Vekic
    TEAM.draw(womens_singles).match('1.29').winner(women.Paolini).in_sets(2)  # ( 30) Sorana Cirstea  OR  (   ) Jasmine Paolini
    ...
    TEAM.draw(womens_singles).match('1.32').winner(women.Bronzetti).in_sets(2)  # (   ) Lucia Bronzetti  OR  (  7) Ons Jabeur
    TEAM.draw(womens_singles).match('1.33').winner(women.Sakkari).in_sets(3)  # (  8) Maria Sakkari  OR  (   ) Karolina Muchova
    TEAM.draw(womens_singles).match('1.34').winner(women.Ponchet).in_sets(3)  # (   ) Nadia Podoroska  OR  (  W) Ponchet
    TEAM.draw(womens_singles).match('1.35').winner(women.Errani).in_sets(3)  # (   ) Sara Errani  OR  (   ) Jil Teichmann
    TEAM.draw(womens_singles).match('1.36').winner(women.Bondar).in_sets(3)  # (   ) Anna Bondar  OR  ( 27) Irina-Camelia Begu
    TEAM.draw(womens_singles).match('1.37').winner(women.Linette).in_sets(3)  # ( 21) Magda Linette  OR  (   ) Leylah Fernandez
    TEAM.draw(womens_singles).match('1.38').winner(women.Sasnovich).in_sets(3)  # (  Q) C.Tauson  OR  (   ) Aliaksandra Sasnovich
    TEAM.draw(womens_singles).match('1.39').winner(women.Jeanjean).in_sets(3)  # (  W) Leolia Jeanjean  OR  (  W) Kimberly Birrell
    TEAM.draw(womens_singles).match('1.40').winner(women.Bencic).in_sets(3)  # (  L) Elina Avanesyan  OR  ( 12) Belinda Bencic
    ...
    TEAM.draw(womens_singles).match('1.50').winner(women.Bonaventure).in_sets(2)  # (   ) Anna Blinkova  OR  (   ) Ysaline Bonaventure
    TEAM.draw(womens_singles).match('1.51').winner(women.Parrizas_Diaz).in_sets(3)  # (   ) Nuria Parrizas Diaz  OR  (  Q) Storm Hunter
    TEAM.draw(womens_singles).match('1.52').winner(women.Trevisan).in_sets(3)  # (   ) Svitolina  OR  ( 26) Martina Trevisan
    TEAM.draw(womens_singles).match('1.53').winner(women.Ostapenko).in_sets(2)  # ( 17) Jelena Ostapenko  OR  (   ) Tereza Martincova
    TEAM.draw(womens_singles).match('1.54').winner(women.Siniakova).in_sets(3)  # (   ) Peyton Stearns  OR  (   ) Katerina Siniakova
    TEAM.draw(womens_singles).match('1.55').winner(women.Parks).in_sets(3)  # (   ) Marketa Vondrousova  OR  (   ) Alycia Parks
    TEAM.draw(womens_singles).match('1.56').winner(women.Kasatkina).in_sets(3)  # (   ) Jule Niemeier  OR  (  9) Daria Kasatkina
    TEAM.draw(womens_singles).match('1.57').winner(women.Pliskova).in_sets(3)  # ( 16) Karolina Pliskova  OR  (   ) Sloane Stephens
    ...
    TEAM.draw(womens_singles).match('1.62').winner(women.Rakhimova).in_sets(3)  # (  Q) Sara Bejlek  OR  (   ) Kamilla Rakhimova
